...
class SaleOrder(models.Model):
    ...
    is_login_user_ceo = fields.Boolean(compute='_compute_is_login_user_ceo')
    ...
    def _compute_is_login_user_ceo(self):
        user = self.env.user
        for order in self:
            order.is_login_user_ceo = user.is_ceo

    def quotations_sent(self):
        self.state = 'sent'

    def action_approved_by_ceo(self):
        for order in self:
            print(self.accumulated_debit, abs(self.accumulated_debit) > 0.001, "===============================")
            if self.partner_id.is_amount_credit_limit and abs(self.accumulated_debit) > 0.001:
    ...
    def reject_sale_order(self):
        ...
        elif self.state == 'approved_by_ceo':
            template_data = {
                'subject': 'Customer credit limit rejected',
                'body_html': """<p>
                Hello %s, <br/><br/>
